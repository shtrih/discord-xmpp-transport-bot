...
class Ram extends EventEmitter {
    constructor(userJID, password) {
        ...
        this.xmppClient = this.xmppClient = new Xmpp.Client({
            jid: userJID,
            password: password,
            bosh: false,
            autostart: false
    ...
    registerListeners() {
        ...
        this.xmppClient.on('stanza', (Stanza) => {
            if (!Stanza.from) {
                LogDebug('Skip stanza w/o "from" ', Stanza);
            ...
            /**
             * @see https://tools.ietf.org/html/rfc6120#section-8.3
             */
