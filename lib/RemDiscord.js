...
class Rem {
    ...
    send(toChannelID, message) {
        return new Promise((resolve) => {
            const channel = this._client.channels.get(toChannelID);
    ...
    async fixMessage(string, message) {
        ...
        const replaceShowflakes = async (match, RID, NID, UID, CID) => {
            if (CID) {
                const channel = this._client.channels.get(CID);
                if (channel) {
                    return '#' + channel.name;
                }
            }
    ...
    editChannel(channelId, topic, byNickname) {
        const ch = this._client.channels.get(channelId);
        if (ch) {
    ...
    canEditChannel(channelId) {
        ...
        if (ch) {
            const permissions = ch.memberPermissions(this._client.user);
